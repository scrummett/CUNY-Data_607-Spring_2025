...
```{r most common positive words in jane austen}
...
The following code is original and not adapted from "Text Mining with R: A Tidy Approach" by Silge & Robinson
...
```{r}
wonder_text <- read_lines("https://www.gutenberg.org/cache/epub/11/pg11.txt")

wonder <- str_c(wonder_text, collapse = " ")

wonder1 <- get_sentences(wonder)
wonder1 <- as.data.frame(unlist(wonder))
class(wonder1)
sentiment(wonder)
